...
# ゼロだった（残る取りこぼしはごく小さく横ばい）。4日だと鮮度を2日捨てるだけ。
                    ...
                    if pace_month not in set(pdf["month"]):
                        ...
                        if asof_day < 1:
                            ...
                            def render_budget_notes():
                                ...
                                with st.expander("▼ この計算の前提"):
                                    st.markdown(
                                        ...
                                        - **{display_names()[1]}と分け合う費用**: 清算タブと同じルールで按分し、
                            ...
                            def render_fixed():
                                ...
                                if abs(fx_gap) >= 1000:
                                    st.caption(
                                        ...
                                        + "　:grey[ⓘ]",
                                        help="都度の支払い（駐車場の都度分・寄附など）が乗ると差が出ます。")
                                else:
                            ...
                            def render_var():
                                ...
                                st.caption(
                                    ":grey[ⓘ 予算の内訳について]",
                                    help=f"予算は「使ってよい上限 − 固定費の着地」= {fyen(total_budget)} を"
                                         "大項目へ割合で配ったもの。"
                                ...
                                st.caption(
                                    ":grey[ⓘ 並び順・「予算未設定」について]",
                                    help="並びは予算の大きい順で固定しています（月の途中で行が入れ替わると"
                                ...
                                st.caption(
                                    ":grey[ⓘ このグラフについて]",
                                    help="固定費は日々コントロールできないので、変動費だけで描いています。"
                                ...
                                st.caption(
                                    ":grey[ⓘ 予算の配分について]",
                                    help="予算の配分（大項目ごとの割合）はローカル版で設定します。"
